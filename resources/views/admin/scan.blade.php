...
@section('title', 'QR / Barcode Operations')
    ...
    <div class="card shadow-sm border-0" style="max-width: 550px; margin: 0 auto;">
        <div class="card-body p-4 text-center">
            <h4 class="fw-bold text-primary mb-1">
                <i class="fa-solid fa-qrcode me-2"></i>QR Code Operations
            </h4>
            <p class="text-muted small mb-3">Stock-In or Out Verification via Scanner</p>

            {{-- Manual / USB Hardware Scanner Input --}}
            <div class="mb-3 text-start">
                <label for="manual_barcode_input" class="form-label fw-semibold">
                    <i class="fas fa-barcode me-1"></i> Manual / USB Barcode Input
                </label>
                <input type="text" id="manual_barcode_input" class="form-control" placeholder="Scan or type barcode here..." autofocus>
            </div>

            <div class="text-muted small mb-2">- OR USE CAMERA SCANNER BELOW -</div>
            ...
            <div id="scanned-result" class="alert alert-info d-none text-start mb-3">
                <div class="fw-bold">
                    <i class="fas fa-check-circle text-success me-1"></i> Scanned Code:
            ...
            {{-- Quantity Input --}}
            <div class="mb-3 text-start">
                <label for="scan_qty" class="form-label fw-semibold">Quantity (အရေအတွက်)</label>
            ...
            <div class="mb-3 text-start" id="expiry_date_container">
                <label for="expiry_date" class="form-label fw-semibold">Expiry Date (သက်တမ်းကုန်ဆုံးရက်)</label>
            ...
            <div class="d-flex gap-2 justify-content-center">
                <button class="btn btn-success fw-bold w-50 py-2" onclick="processStock('IN')">
                    <i class="fa-solid fa-arrow-down me-1"></i> Scan Stock-In
                </button>
                <button class="btn btn-danger fw-bold w-50 py-2" onclick="processStock('OUT')">
                    <i class="fa-solid fa-arrow-up me-1"></i> Scan Dispatch (OUT)
                </button>
            </div>
        </div>
    </div>
...
<script>
    ...
    function processStock(type) {
        ...
        if (type === 'IN' && !expiryDate) {
            alert('ကျေးဇူးပြု၍ Expiry Date ထည့်သွင်းပေးပါ။');
            return;
        }
        ...
        .then(data => {
            if (data.success) {
                alert(`SUCCESS: ${data.message}\nItem: ${data.item_name}\nNew Stock Level: ${data.new_balance}`);
